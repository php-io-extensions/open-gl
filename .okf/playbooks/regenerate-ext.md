...
tags: [opengl, playbook, packaging]
status: draft
...
4. Commit regenerable artifacts that belong in git (`ext/` sources, `config.m4`, `ide/0.5.0/` stubs) — not phpize junk.
...
- Always run REGISTER fixup for `Opengl\GL\*` — wired into `prepare-ext.sh`; see [REGISTER truncation](/traps/register-class-truncation.md).[^fixup][^prepare]
- Verify Darwin framework and Linux `-lGL` paths both configure cleanly when touching `config.m4`.
- Smoke: `class_exists('Opengl\\GL\\GlBuffer')` (not bare `Buffer`).
