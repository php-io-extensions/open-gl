#!/usr/bin/env bash
# Generate Zephir C sources and rewrite ext/config.m4 for portable OpenGL.
set -Eeuo pipefail
...
# Sync C headers into ext/ (PIE builds from ext/ only).
mkdir -p ext/src
cp -f src/*.h ext/src/
...
rm -rf Makefile Makefile.fragments Makefile.objects Makefile.global modules .libs autom4te.cache \
    ...
    opengl.so opengl.la opengl.lo opengl.o opengl.dep 2>/dev/null || true
rm -rf opengl.so.dSYM .libs 2>/dev/null || true
